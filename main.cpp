...
#include <algorithm>
#include <cmath>
#include "src/algorithms/mapa.h"
#include "src/algorithms/ksiegi.h"
#include "src/algorithms/otoczka.h"
#include "src/algorithms/dekametrowcy.h"
#include "src/models/kopalnia.h"
#include "src/models/krasnoludek.h"
...
int main(int argc, char *argv[])
{
    ...
    if (komenda == "GUI_DATA_DUMP")
    {
        ...
        {
            if (linia.empty() || linia.find("ID") != string::npos)
                continue;
            stringstream ss(linia);
            string id_str, id_kop_str, mineraly_str, x_str, y_str;
            getline(ss, id_str, ',');
            getline(ss, id_kop_str, ',');
            getline(ss, mineraly_str, ',');
            getline(ss, x_str, ',');
            getline(ss, y_str, ',');
            try
            {
        ...
        {
            vector<Wspolrzedne> coords;
            coords.reserve(punktyDoOtoczkiWithID.size());
            for (auto &pp : punktyDoOtoczkiWithID)
                coords.push_back(pp.first);
        ...
        if (argc >= 5 && string(argv[2]) == "SALWA")
        {
            try
            {
                ...
                if (straznicyPunkty.size() >= 1)
                {
                    ...
                    if (prawy_indeks < 0)
                        prawy_indeks = 0;
                    if (lewy_indeks >= (int)otoczka.size())
                        lewy_indeks = 0;
                    if (prawy_indeks >= (int)otoczka.size())
