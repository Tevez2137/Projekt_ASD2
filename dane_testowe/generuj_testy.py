...
def losuj_punkt_unikatowy(wymiary, zajete_punkty, min_odstep=MIN_ODSTEP):
    while True:
        x = random.randint(50, wymiary - 50)
        y = random.randint(50, wymiary - 50)
        if all(abs(x - px) >= min_odstep or abs(y - py) >= min_odstep for px, py in zajete_punkty):
            zajete_punkty.add((x, y))
            return x, y
...
with open('./kopalnie_wielkie.csv', 'w', encoding='utf-8') as f:
    ...
    for i in range(1, ILE_KOPALNI_WIELKIE + 1):
        x, y = losuj_punkt_unikatowy(WYMIARY_MAPY, zajete_punkty)
